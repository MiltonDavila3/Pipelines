...
public class PaymentController(PaymentsService paymentsService, StoreContext context,
    IConfiguration configuration, ILogger<PaymentController> logger) : BaseApiController
{
    [Authorize]
    [HttpPost]
    public async Task<ActionResult<BasketDto>> CreateOrUpdatePaymentIntent()
    {
        ...
        var intent = await paymentsService.CreateUpdatePaymentIntent(basket);
    ...
    private Event ConstructStripeEvent(string json)
    {
        try
        {
            ...
            Request.Headers["Stripe-Signature"], configuration["StripeSettings:WhSecret"]);
        }
        catch (Exception ex)
        {
